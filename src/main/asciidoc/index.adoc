...
[source,$lang]
----
{@link examples.MailExamples#createSharedClient}
----
...
[source,$lang]
----
{@link examples.MailExamples#createNonSharedClient}
----
...
[source,$lang]
----
{@link examples.MailExamples#mailMessage}
----
...
[source,$lang]
----
{@link examples.MailExamples#attachment}
----
...
[source,$lang]
----
{@link examples.MailExamples#inlineAttachment}
----
...
To be able to perform DKIM sign, it needs to go through the email to calculate the body hash, including attachments.
If an attachment comes from a ReadStream, it won't be able to go through again. This client provides 2 strategies to
cache the attachment data.

* Caching in memory

By default, it tries to cache the data from attachment stream to a buffer so that it can be used for real email sending.
...
* `pubSecKeyOptions` *required*, List of https://vertx.io/docs/apidocs/io/vertx/ext/auth/PubSecKeyOptions.html[PubSecKeyOptions], used to specify the RSA https://www.ietf.org/rfc/rfc5208.txt[PKCS#8] format private key.
...
* `auid` *required*, String, the Agent or User Identifier(AUID),
